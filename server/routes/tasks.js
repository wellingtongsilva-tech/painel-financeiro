...
// ?scope=today | open | all  (padrão: open)
router.get('/', (req, res) => {
  const scope = req.query.scope || 'open';
  let rows;
  if (scope === 'all') {
    rows = db
      .prepare('SELECT * FROM tasks WHERE user_id = ? ORDER BY done, due_date IS NULL, due_date, id DESC')
      .all(req.user.id);
  } else if (scope === 'today') {
    ...
    rows = db
      .prepare('SELECT * FROM tasks WHERE user_id = ? AND done = 0 ORDER BY due_date IS NULL, due_date, id')
      .all(req.user.id);
  }
  res.json(rows);
});
