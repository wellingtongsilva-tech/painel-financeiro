...
router.get('/', (req, res) => {
  ...
  const tasks = db
    ...
    .all(uid, day);
  const overdue = db
    .prepare('SELECT COUNT(*) AS n FROM tasks WHERE user_id = ? AND done = 0 AND due_date < ?')
    .get(uid, day).n;

  // --- Finanças do mês ---
  const monthRows = db
    .prepare('SELECT type, amount FROM transactions WHERE user_id = ? AND substr(date,1,7) = ?')
    .all(uid, month);
  let entradas = 0,
    saidas = 0;
